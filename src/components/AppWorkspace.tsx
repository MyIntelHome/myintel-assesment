...
import {familyTemplateFor,activeHomeSpaces} from "@/domain/home-profile";
import {buildCaseView} from "@/lib/selectors";
import {FamilyFlow} from "./FamilyFlow";
import {ProfessionalPortal,ProfessionalAccessReview} from "./ProfessionalPortal";
import {isClinicalRecord,type ProfessionalAccess} from "@/domain/access";
...
export default function AppWorkspace(){
  const [session,setSession]=useState<{user:AccountUser|null;paymentsEnabled:boolean;professionalAccess?:ProfessionalAccess|null}|null>(null);
  const [professional,setProfessional]=useState(false);
  useEffect(()=>{setProfessional(new URLSearchParams(window.location.search).get("portal")==="professional")},[]);
  ...
  if(professional)return <ProfessionalPortal user={session.user} access={session.professionalAccess??null}/>;
...
function Workspace({user,paymentsEnabled}:{user:AccountUser|null;paymentsEnabled:boolean}){
  ...
  const saved=api.cases.filter(c=>c.audience!=="unchosen" && !isClinicalRecord(c));
  ...
  return <div className="app-shell">
    ...
    <div className="app-status no-print"><span>Private review · use example information</span><span role="status">{(!api.hydrated || api.saveState==="saving") && <span className="home-save-spinner" aria-hidden="true"/>}{api.hydrated?saveLabel:"Opening your assessments…"}</span></div>
    ...
    {(view==="home" || view==="assessments") && <main className="overview">
      ...
      <section className="path-card clinical-path"><span className="path-label">SUPPORT WHEN YOU NEED IT</span><h2>You don’t have to<br/>work it out alone.</h2><p>Ask for an occupational therapist, practical home changes or help choosing technology. MyIntel helps you work out the next step.</p><button className="app-secondary" onClick={()=>requestHelp("professional_assessment")}>Explore professional support →</button><span className="path-note">Your choice · Share your home check only with consent</span></section></div>}
      <section className="recent-section"><div className="section-heading"><h2>{view==="home"?"Pick up where you left off":"Saved assessments"}</h2>{view==="assessments" && <div className="inline-actions"><button className="app-secondary" disabled={!ready} onClick={()=>begin("family")}>New home check</button></div>}</div>
      {saved.length===0?<div className="empty-work"><strong>Your next step starts here.</strong><p>Once you begin, your assessments will appear here so you can return to them.</p></div>:<div className="case-grid">{saved.slice().reverse().slice(0,view==="home"?4:100).map(c=>{const progress=c.audience==="family"?familyProgress(activeHomeSpaces(c.spaces).map(s=>({spaceId:s.id,template:familyTemplateFor(s)})),c.familyAnswers).percent:buildCaseView(c).completeness.percent;return <button disabled={!ready} key={c.id} className="case-card" onClick={()=>{api.openCase(c.id);navigate("workspace")}}><span className="case-type">{c.audience==="family"?"Home check":"Clinical assessment"}</span><strong>{labelCase(c)}</strong><span>{c.signoff.signedAt?"Signed · "+c.reportVersions.length+" report version(s)":progress+"% of questions completed"}</span><span className="case-progress" aria-hidden="true"><i style={{width:progress+"%"}}/></span><span className="case-return">{c.signoff.signedAt?"View report":"Continue assessment"} →</span></button>})}</div>}
      </section>
    ...
    {view==="workspace" && <><div className="workspace-breadcrumb no-print"><button onClick={()=>navigate("assessments")}>← My assessments</button><span>{api.state.audience==="clinician"?"Clinical workspace":"Home check"}</span><button onClick={()=>requestHelp()}>Get professional help</button></div>{!isClinicalRecord(api.state)?<FamilyFlow key={api.state.id} api={api} onRequestHelp={requestHelp}/>:<main className="connection-state"><h1>This record needs professional access</h1><p>Your clinical record is preserved. Open the professional sign-in to request access.</p></main>}</>}
    {view==="help" && <><ProfessionalHelp key={helpService??"any"} user={user} initialService={helpService} homeCaseId={api.state.audience==="family" && activeHomeSpaces(api.state.spaces).length>0 && ready && user?api.state.id:undefined} onBack={()=>navigate("home")} onRequests={()=>navigate("requests")}/>{providers.length>0 && <section className="overview"><h2>Professionals in the MyIntel network</h2><p>Availability and fit are confirmed after your request.</p><div className="case-grid">{providers.map(p=><article className="provider-card" key={p.id}><span className="case-type">{SERVICES[p.service].title}</span><h3>{p.name}</h3><p>{p.credentials}</p><p>{p.area}</p><span className="verified-label">Reviewed by MyIntel</span></article>)}</div></section>}</>}
    {view==="requests" && <RequestCenter user={user} paymentsEnabled={paymentsEnabled} onNew={()=>requestHelp()}/>}
    {view==="operations" && (user?.isAdmin?<><ProfessionalAccessReview/><Operations/></>:<main className="connection-state"><h1>MyIntel staff access required</h1><button className="app-primary" onClick={()=>navigate("home")}>Back to overview</button></main>)}
    {view==="account" && <main className="account-page"><p className="eyebrow">YOUR MYINTEL ACCOUNT</p><h1>{user?"Your work, ready when you are.":"Keep your next steps together."}</h1>{user?<><section className="account-card"><span className="avatar large">{user.name.slice(0,1).toUpperCase()}</span><div><h2>{user.name}</h2><p>{user.email}</p><span>{accountProviderLabel}</span></div></section><div className="account-grid"><section className="account-card"><h2>Assessments & requests</h2><p>Your account saves your assessments and professional requests for your next visit. Your assessments are private to this account; MyIntel staff can review service requests.</p><button className="app-secondary" onClick={()=>navigate("assessments")}>View my assessments</button></section><section className="account-card"><h2>Earlier device drafts</h2><p>Copy drafts from the previous version in this browser into your account. Only home checks are imported here. Clinical drafts stay available through the professional workspace.</p><button className="app-secondary" disabled={!ready} onClick={()=>{api.importLocal();navigate("assessments")}}>Import device drafts</button></section></div><p className="account-note">{accountNotice}</p>{ready?<a className="app-secondary signout" href={signOutHref} target="_top">Sign out</a>:<p>Wait for your latest changes to save before signing out.</p>}</>:<section className="account-card"><h2>Sign in to save to your account</h2><p>Return to your assessments and track requests for professional help. A home check does not require sign-in to show your results.</p><a className="app-primary" href={signInHref("/?view=account")} target="_top">{signInLabel}</a><p className="account-note">Already started on this device? You can import your draft after signing in.</p></section>}</main>}
    </>}
    </div><footer className="app-footer no-print"><span>MyIntel · A practical path to living well at home</span><div className="inline-actions"><button onClick={()=>navigate("help")}>Need help?</button>{ready && <a href="/?portal=professional">Professional sign-in</a>}</div></footer>
  </div>;
}
