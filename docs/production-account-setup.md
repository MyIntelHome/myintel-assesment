...
The app accepts both token-hash confirmation links and Supabase's default browser-fragment session links. For a default link, the confirmation page moves the provider tokens into page memory and removes the fragment from browser history immediately; the server still verifies the provider identity before creating a MyIntel session. Custom token-hash templates are preferred because the confirmation page consumes them only after an explicit POST, so a GET-only email scanner does not consume the token. Signup and recovery pass the canonical application origin as `redirectTo`; the saved templates append `/auth/confirm` and the token query. This also makes a dashboard resend work when Supabase falls back to the configured Site URL. During staging the Site URL is the stable pilot origin; change it to the production origin only with the tested production cutover. Referrer policy is no-referrer. Do not log confirmation query strings, tokens, request bodies, fragments, or cookies in external analytics.
...
1. Resolve vendor/privacy/retention decisions. Supabase project `yyueldzkxdcykljbdquz` is healthy in East US (Ohio); email confirmation is enabled, the production and stable pilot-branch callbacks are allowlisted, and private bucket `home-photos` limits uploads to 3 MB JPEGs. Resend has verified `myintelhome.com`, and Supabase custom SMTP is enabled with `accounts@myintelhome.com` and a restricted Resend sending key. Administrator confirmation and first sign-in succeeded; the complete recovery journey remains a live gate.
