...
- The stable pilot-branch callback is allowlisted in Supabase. Nine branch-only Vercel variables connect it to Turso, Supabase Auth, the private `home-photos` bucket and `austin@myintelhome.com` as the verified-email staff administrator. Resend has verified `myintelhome.com`; Supabase custom SMTP sends as `accounts@myintelhome.com`. Signup and recovery templates now honor the full `redirectTo` callback supplied by the app, after the first signup message incorrectly used the production Site URL and reached a 404. A corrected confirmation email was accepted for delivery, but confirmation, first sign-in and recovery remain to be completed.
