...
Professionals are not prepopulated. MyIntel must verify credentials, service area and listing consent before adding anyone. Requests appear in the operations queue; email/SMS alerts, automatic matching, availability and confirmed booking are not connected. Family users may explicitly attach their saved home-check summary, layout and routine answers to an inquiry, then optionally share guided photos with MyIntel. Images use private R2 storage and authenticated access. Nothing is automatically forwarded to a professional; clinical assessment archives remain separate.
